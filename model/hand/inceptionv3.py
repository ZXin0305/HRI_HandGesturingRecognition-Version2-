...
class InceptionV3(nn.Module):
    ...
    def __init__(self, in_ch=3, num_classes=100):
        super().__init__()
        self.Conv2d_1a_3x3 = BasicConv2d(in_ch, 32, kernel_size=3, padding=1)
...
if __name__ == "__main__":
    ...
    net1.to(device)
    net2.to(device)
    input1 = torch.ones(size=(1, 1, 160, 160), dtype=torch.float32)
    avg_pool = nn.FractionalMaxPool2d(kernel_size=3, output_ratio=(1 / 5, 1 / 5))
    input2 = torch.ones(size=(1, 1, 32 ,32), dtype=torch.float32)
    input1 = input1.to(device)
    input2 = input2.to(device)
    st = time()
    input1 = avg_pool(input1)
    # et = time()
    # print(et - st)
    out1 = net1(input1)
    out2 = net2(input2)
