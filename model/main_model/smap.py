...
class Upsample_unit(nn.Module): 
    ...
    def __init__(self, ind, in_planes, up_size, output_chl_num, output_shape,
        ...
        self.residual_connection_new = ResidualPool(in_ch=chl_num, out_ch=chl_num)  #重新训练之后,这里开了 0325
        self.relu = nn.ReLU(inplace=True)
        self.CA = CoordAtt(chl_num, chl_num)
    ...
    def forward(self, x, up_x):
        ...
        if self.ind > 0:
            # out = self.residual_connection_new(out)  #重新训练之后,这里开了 0325
...
class SMAP(nn.Module):
    ...
    def __init__(self, cfg, run_efficient=False, **kwargs):
        ...
        self.cross_stage_conv = conv_bn_relu(in_planes=64, out_planes=64, kernel_size=3, stride=1, padding=1, has_bn=True, has_relu=False)
