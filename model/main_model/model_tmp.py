...
class SMAP_tmp(nn.Module):
    
    def __init__(self, cfg, run_efficient=False, **kwargs):
        super(SMAP_tmp, self).__init__()
    ...
    def _calculate_loss(self, outputs, valids, labels, rdepth_map, rdepth_mask):
        ...
        for i in range(self.stage_num):
            for j in range(4):  # multi-scale
                ...
                depth_att_loss = loss_depth_att(outputs['root_d_att_maps'][i][j], rdepth_mask).mean()
                ...
                tmp_loss = 0.1 * tmp_loss_2d + 2 * tmp_loss_3d + 4 * depth_loss + 0.5 * depth_att_loss # 12.7
                if j < 3:
                    tmp_loss = tmp_loss / 4
