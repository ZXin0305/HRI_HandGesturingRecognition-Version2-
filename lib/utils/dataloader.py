...
def get_train_loader(cfg, num_gpu, is_dist=True, is_shuffle=True, start_iter=0, 
    ...
    class BatchCollator(object):
        ...
        def __call__(self, batch):
            ...
            # rdepth_map = torch.stack(transposed_batch[3], dim=0)
            # rdepth_mask = torch.stack(transposed_batch[4], dim=0)
            # return images, valids, labels, rdepth_map, rdepth_mask
...
def get_test_loader(cfg, num_gpu, local_rank, stage, use_augmentation=False, with_mds=False):
    ...
    # -------- make samplers -------- #
    # sselect the sample follow the order
    sampler = torch.utils.data.sampler.SequentialSampler(dataset)
    ...
    class BatchCollator(object):
        ...
        def __call__(self, batch):
            transposed_batch = list(zip(*batch))
            ori_img = torch.stack(transposed_batch[0], dim=0)
            net_input_img = torch.stack(transposed_batch[1], dim=0)
            meta_data = torch.stack(transposed_batch[2], dim=0)
            img_path = transposed_batch[3]
            scale = transposed_batch[4]
            return ori_img, net_input_img, meta_data, img_path, scale
